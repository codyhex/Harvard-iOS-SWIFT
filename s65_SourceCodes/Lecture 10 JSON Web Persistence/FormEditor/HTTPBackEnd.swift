...
class HTTPBackEnd {
    ...
    let host = "data.classyswift.com"
    
    /* The endpoint within the web server */
    let endPoint = "/lib/ajax/data.php"
    ...
    var session: NSURLSession = {
        Util.log("creating URL session with 15.0s timeout")
    ...
    var staticURL: String {
        get {
            ...
            Util.log("computed URL \(url) from \(networkProtocol), \(host), and \(endPoint)")
            return url
        }
    }
    ...
    func encode(vals: [String: String]) -> String {
        ...
        Util.log("Returning \(result) from dictionary: \(vals)")
        return result
    }
    ...
    */
    func httpRequest(url: NSURL, rawDataDidArrive: (data: NSData?, message: String?) -> Void) {
        ...
        let task = session.dataTaskWithURL(url) {
            ...
            has completed */
            Util.log("data returned in closure: Error: \(netError?.localizedDescription ?? noErr)")
            var errmsg: String?
            ...
            NSThread.sleepForTimeInterval(5.0) // 'fake' delay; simulating a slow network
        ...
        Util.log("Requesting HTTP task to start on network queue")
