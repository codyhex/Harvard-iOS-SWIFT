...
protocol ColoredSquareDataSource: class /* adopters must be class (reference type) */ {
    ...
    // subscript defines what the [] syntax does on the class
...
class ColoredSquareModel: ColoredSquareDataSource {
    ...
    subscript(index: String) -> Double? {
        ...
        // suppose index = "sideLength", newValue = 25.0
        set {
            var successFlag = false
            // STEP 3 of diagram (A, B, C below): validate data
            if let attr = values[index], var unwrappedValue = newValue {
                // continuing example, attr is the 'sideLength' attribute
                // attr knows about the minimum, maximum, and current values of
                // the square side's length
    ...
    func notifyObservers(#success: Bool) {
        ...
        // postNOtification returns immediately, helping us to finish event processing quickly
    }
}
