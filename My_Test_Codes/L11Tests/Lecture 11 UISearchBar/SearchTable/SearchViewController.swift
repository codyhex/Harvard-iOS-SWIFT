...
class SearchViewController: ObservingViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate {
    ...
    func updateUI() {
        ...
        searchTableView.reloadData() // (6) Magic happens, setNeedsDisplay
    }
    
    override func viewDidLoad() {
        super.viewDidLoad()
        for m in models {
            // this is the observerLL
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell
    {
        ...
        cell!.textLabel!.text = model.valueFromSection(indexPath.section, atIndex: indexPath.row)?.listDescription // (7)
        return cell!
    }
