...
let NUM_PANELS = CGFloat(7.0)
let ITEM_SIZE = CGFloat(150.0)
...
class CircularLayout: UICollectionViewLayout {
    ...
    override func layoutAttributesForItemAtIndexPath(indexPath: NSIndexPath) -> UICollectionViewLayoutAttributes! {
        ...
        var t = (cv.contentOffset.x - cv.frame.size.width*1.0) / (cv.frame.size.width*1.0)
        var angle = t * CGFloat(M_PI)
        var attributes = UICollectionViewLayoutAttributes(forCellWithIndexPath: indexPath)
        attributes.size = CGSizeMake(ITEM_SIZE, ITEM_SIZE)
        var hArcLength = cosf((2.0 * Float(indexPath.item) * Float(M_PI) / Float(cellCount)) + Float(2.0*angle))
        var cx = cv.contentOffset.x + radius * CGFloat(hArcLength) + cv.frame.width/2.0
        var vArcLength = sinf((2.0 * Float(indexPath.item) * Float(M_PI) / Float(cellCount)) + Float(2.0*angle))
