...
class ViewController: UIViewController {
    ...
    @IBAction func numericValueDidChange(sender: UITextField) {
        ...
        // "sender.text.toDouble()" is the UI->Internal logic translation
    ...
    */
    ...
    // After view rendered on screen, all frames are set.
    override func viewDidAppear(animated: Bool) {
        super.viewDidAppear(animated)
        let frame = squareView.frame
        /** UIBible "A)": gaining access to the models it needs **/
        // Normally we initialize the model in viewDidLoad() [earlier]
        // but frame data is not yet ready then
    ...
    func startModelListener() {
        ...
        observer = center.addObserverForName(ModelMsgs.notificationName, object: model, queue: uiQueue) {
            [unowned self]
            (notification) in
            if let csm = notification.object as? ColoredSquareModel {
                let x = csm["xLoc"]!
                println("notification received, model is: \(csm) x is \(x)")
            }
