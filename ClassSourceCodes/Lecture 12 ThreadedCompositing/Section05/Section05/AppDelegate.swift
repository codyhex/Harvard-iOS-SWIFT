...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    var threadPipeQueue = ThreadSafeQueue()
    var udpReceiverStream:NSInputStream!
    ...
    {
        autoreleasepool {
            var localRunLoop = NSRunLoop.currentRunLoop()
            ...
            while self.shouldStopBackgroundThread == false {
                autoreleasepool {
                    var date = NSDate().dateByAddingTimeInterval(0.1)
                    localRunLoop.runMode(NSDefaultRunLoopMode, beforeDate: date)
                    while threadPipeQueue.count > 0 {
                        var op = threadPipeQueue.dequeueObject()
                        if let operation = op {
                            operation.execute()
                        }
                    }
